...
User review $file / $focus / $0 / $ARGUMENTS[1] / $ARGUMENTS / $filename`), 0o644))
	...
	require.NoError(t, os.WriteFile(filepath.Join(workspace, ".omc", "commands", "oh-my-claudecode", "hud.md"), []byte("OMC HUD $ARGUMENTS"), 0o644))
...
Deploy ${CLAUDE_PLUGIN_ROOT} ${CLAUDE_PLUGIN_DATA} $ARGUMENTS`), 0o644))
	...
	require.Contains(t, commandNames(commands), "oh-my-claudecode:hud")
	...
	command, err = Find(configHome, workspace, "/oh-my-claudecode:hud")
	require.NoError(t, err)
	require.Equal(t, "oh-my-claudecode:hud", command.Name)
	require.Equal(t, "omc", command.Source)
	require.Equal(t, "OMC HUD session", Render(command, "session").Rendered)
	...
	requireCommandSource(t, sources, "omc", filepath.Join(workspace, ".omc", "commands"), true)
