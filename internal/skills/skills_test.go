...
func TestCompatibilityProjectSkillRoots(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(filepath.Join(repo, ".codex", "skills", "port"), 0o755))
	require.NoError(t, os.MkdirAll(filepath.Join(parent, ".agents", "skills", "rogue"), 0o755))
	...
	require.NoError(t, os.WriteFile(filepath.Join(repo, ".codex", "skills", "port", "SKILL.md"), []byte("Codex port skill"), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(parent, ".agents", "skills", "rogue", "SKILL.md"), []byte("Rogue parent skill"), 0o644))
	...
	sources := Sources(configHome, workspace)
	codexSource := sourceByPath(sources, filepath.Join(repo, ".codex", "skills"))
	...
	require.Equal(t, DiscoveryRoot{}, sourceByPath(sources, filepath.Join(parent, ".agents", "skills")))
