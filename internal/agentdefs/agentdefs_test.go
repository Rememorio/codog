...
func TestLoadAgentDefinitions(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(pluginDir, "plugin.json"), []byte(`{"id":"demo","name":"demo","agents":["./extra/critic.json"]}`), 0o644))
	...
	require.NoError(t, os.WriteFile(filepath.Join(pluginDir, "extra", "critic.json"), []byte(`{"description":"plugin critic","tools":["grep"]}`), 0o644))

	defs, err := Load(workspace)
	require.NoError(t, err)
	require.Len(t, defs, 3)
	require.Equal(t, "demo:critic", defs[0].Name)
	require.Equal(t, "plugin:demo", defs[0].Source)
	require.Equal(t, "demo", defs[0].Plugin)
	require.Equal(t, []string{"grep"}, defs[0].Tools)
	require.Equal(t, "demo:helper", defs[1].Name)
	require.Equal(t, "plugin:demo", defs[1].Source)
	require.Equal(t, "demo", defs[1].Plugin)
	require.Equal(t, []string{"read_file"}, defs[1].Tools)
	require.Equal(t, "reviewer", defs[2].Name)
	require.Equal(t, "workspace", defs[2].Source)
	require.Equal(t, []string{"grep"}, defs[2].Tools)
}
