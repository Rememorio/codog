...
func TestControlMCPEndpoints(t *testing.T) {
	...
	body, err = io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Contains(t, string(body), `"kind":"mcp_auth"`)
	require.Contains(t, string(body), `"server":"remote"`)
	require.Contains(t, string(body), `"phase":"ready"`)
	require.Contains(t, string(body), `"tool_count":1`)
	require.Contains(t, string(body), `"resource_count":1`)
	require.NotContains(t, string(body), "secret")
	...
	req, err := http.NewRequest(http.MethodPut, controlServer.URL+"/mcp/auth", nil)
	require.NoError(t, err)
	resp, err = http.DefaultClient.Do(req)
	require.NoError(t, err)
	defer func() { _ = resp.Body.Close() }()
