...
func TestRegistryV1IsSelfDescribing(t *testing.T) {
	...
	require.Contains(t, fieldIDs(registry), "new_items[].dedupe_key")
	require.Contains(t, fieldIDs(registry), "new_items[].same_root_cause[]")
	require.Contains(t, fieldIDs(registry), "changed_items[].dedupe_key")
	require.Contains(t, fieldIDs(registry), "changed_items[].same_root_cause[]")
