...
func TestReportBackpressureToolCollapsesRepeatedRoadmapReports(t *testing.T) {
	...
	secondOut, err := reportTool.Execute(context.Background(), []byte(`{"action":"generate","channel":"dogfood","trigger_id":"nudge-cycle-1","checked_surfaces":["roadmap","sessions"],"now":"2026-07-07T13:02:00Z"}`))
	require.NoError(t, err)
