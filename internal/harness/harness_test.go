...
func TestRunUsesMockProvider(t *testing.T) {
	...
	require.Equal(t, 12, privacyKeybindings.RequestCount)
	...
	require.Contains(t, privacyKeybindings.Output, `"opened":true`)
	require.Contains(t, privacyKeybindings.Output, `"valid":true`)
	require.Contains(t, privacyKeybindings.Output, `"contexts":7`)
	require.Contains(t, privacyKeybindings.Output, `"bindings":86`)
	require.Contains(t, privacyKeybindings.Output, `"shift_enter":true`)
	require.Contains(t, privacyKeybindings.Output, `"prompt_stash_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"external_editor":true`)
	require.Contains(t, privacyKeybindings.Output, `"external_editor_chord":true`)
	require.Contains(t, privacyKeybindings.Output, `"kill_agents_chord":true`)
	require.Contains(t, privacyKeybindings.Output, `"compact_chord":true`)
	require.Contains(t, privacyKeybindings.Output, `"undo_change_chord":true`)
	require.Contains(t, privacyKeybindings.Output, `"export_chord":true`)
	require.Contains(t, privacyKeybindings.Output, `"copy_chord":true`)
	require.Contains(t, privacyKeybindings.Output, `"attachment_remove_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"composer_undo_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"clipboard_paste_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"quick_open_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"global_search_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"runtime_control_keys":true`)
	require.Contains(t, privacyKeybindings.Output, `"mode_cycle_keys":true`)
	require.Contains(t, privacyKeybindings.Output, `"modal_navigation_keys":true`)
	require.Contains(t, privacyKeybindings.Output, `"attachment_nav_keys":true`)
	require.Contains(t, privacyKeybindings.Output, `"diff_dialog_keys":true`)
	require.Contains(t, privacyKeybindings.Output, `"message_actions_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"transcript_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"terminal_keys":true`)
	require.Contains(t, privacyKeybindings.Output, `"background_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"todo_panel_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"task_board_key":true`)
	require.Contains(t, privacyKeybindings.Output, `"queue_edit_key":true`)
	...
	tuiPrompt := findScenario(t, report, "tui_prompt_completion_roundtrip")
	require.True(t, tuiPrompt.OK)
	...
	require.Contains(t, tuiPrompt.Output, `"undo_preview":true`)
	require.Contains(t, tuiPrompt.Output, `"attachment_preview":true`)
	require.Contains(t, tuiPrompt.Output, `"attachment_navigation":true`)
	...
	require.Contains(t, tuiPrompt.Output, `"global_search_preview":true`)
	require.Contains(t, tuiPrompt.Output, `"global_search":true`)
	require.Contains(t, tuiPrompt.Output, `"diff_dialog":true`)
	require.Contains(t, tuiPrompt.Output, `"model_picker":true`)
	require.Contains(t, tuiPrompt.Output, `"runtime_fast_toggle":true`)
	require.Contains(t, tuiPrompt.Output, `"runtime_fast_badge":true`)
	...
	require.Contains(t, tuiPrompt.Output, `"custom_keybinding_quick_open":true`)
	require.Contains(t, tuiPrompt.Output, `"custom_keybinding_editor":true`)
	...
	require.Equal(t, 3, remoteControl.Total)
	require.ElementsMatch(t, []string{"ssh_print_plan_roundtrip", "remote_api_listener_roundtrip", "remote_trigger_roundtrip"}, remoteControl.Scenarios)

	sshPrint := findScenario(t, report, "ssh_print_plan_roundtrip")
	require.True(t, sshPrint.OK)
	require.Equal(t, "remote-control", sshPrint.Category)
	require.Equal(t, 0, sshPrint.ToolCalls)
	require.Equal(t, 1, sshPrint.RequestCount)
	require.Equal(t, "ssh print plan harness ok", sshPrint.FinalMessage)
	require.Contains(t, sshPrint.Output, `"kind": "ssh"`)
	require.Contains(t, sshPrint.Output, `"print": true`)
	require.Contains(t, sshPrint.Output, `"prompt_configured": true`)
	require.Contains(t, sshPrint.Output, `"prompt"`)
	require.NotContains(t, sshPrint.Output, `"repl"`)
...
func TestScenarioManifestMatchesRunScenarios(t *testing.T) {
	...
	sshPrint := findManifestScenario(t, manifest, "ssh_print_plan_roundtrip")
	require.Equal(t, "remote-control", sshPrint.Category)
	require.Contains(t, sshPrint.ParityRefs, "SSH remote sessions")
	require.Contains(t, sshPrint.ParityRefs, "Headless print mode")
