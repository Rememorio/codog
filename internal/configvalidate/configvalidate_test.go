...
func TestValidateBytesAcceptsRemoteSection(t *testing.T) {
	result := ValidateBytes([]byte(`{"remote":{"enabled":true,"authToken":"token","leaseSeconds":60}}`), "config.json")
...
func TestValidateBytesAcceptsEnterpriseSection(t *testing.T) {
	result := ValidateBytes([]byte(`{"enterprise":{"policy":"policy.json","policyPublicKey":"key"}}`), "config.json")
...
func TestValidateBytesAcceptsEditorBridgeSection(t *testing.T) {
	result := ValidateBytes([]byte(`{"editor_bridge":{"socket":"codog.sock","authToken":"token"}}`), "config.json")
...
func TestValidateBytesAcceptsUpdaterSection(t *testing.T) {
	result := ValidateBytes([]byte(`{"updater":{"manifestURL":"https://updates.example/manifest.json"}}`), "config.json")
