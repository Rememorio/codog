...
func TestRunDryRunPlansSafeBranchAndPR(t *testing.T) {
	...
	require.NotNil(t, report.Ship)
	require.Equal(t, "planned", report.Ship.Status)
	require.Equal(t, "pull_request", report.Ship.Provenance.MergeMethod)
	require.Equal(t, "Codog Test <codog@example.test>", report.Ship.Provenance.Actor)
	require.Len(t, report.Ship.Events, 5)
	require.Len(t, report.ShipEvents, 5)
	require.Equal(t, "ship.provenance", report.ShipEvents[4].LaneEvent)
...
func TestRunCommitsAndPushesWithoutPR(t *testing.T) {
	...
	require.NotNil(t, report.Ship)
	require.Equal(t, "confirmed", report.Ship.Status)
	require.Equal(t, "branch_push", report.Ship.Provenance.MergeMethod)
	require.Equal(t, report.Commit, report.Ship.Provenance.FirstCommit)
	require.Equal(t, report.Commit, report.Ship.Provenance.LastCommit)
	require.Equal(t, report.Commit, report.Ship.Provenance.CommitRange)
	require.Equal(t, 1, report.Ship.Provenance.CommitCount)
	require.Equal(t, 1, report.Ship.Classification.Intentional)
	require.Equal(t, 0, report.Ship.Classification.Riders)
	require.Len(t, report.ShipEvents, 5)
	require.Equal(t, "ship.pushed_main", report.ShipEvents[3].LaneEvent)
